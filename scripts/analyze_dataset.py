...
from src.analysis.train_val_comparison import (
    TrainValComparison
)
...
def analyze_split(
    ...
    class_dist = (
        class_stats.class_distribution()
    )
    ...
    print(
        bbox_stats.area_summary()
    )

    print(
        bbox_stats.size_distribution()
    )

    plot_bbox_area_histogram(
        df,
        figure_dir /
        f"{split_name}_bbox_area_histogram.png"
    )

    plot_bbox_area_log_histogram(
        df,
        figure_dir /
        f"{split_name}_bbox_area_log_histogram.png"
    )

    plot_aspect_ratio_histogram(
        df,
        figure_dir /
        f"{split_name}_aspect_ratio_histogram.png"
    )

    occ_stats = OcclusionStats(df)

    print(
        occ_stats.overall_occlusion_rate()
    )

    print(
        occ_stats.truncation_rate()
    )
    ...
    plot_occlusion_distribution(
        df,
        figure_dir /
        f"{split_name}_occlusion_distribution.png"
    )

    plot_occlusion_by_class(
        occ_df,
        figure_dir /
        f"{split_name}_occlusion_by_class.png"
    )

    scene_stats = SceneStats(df)

    weather_df = (
        scene_stats.weather_distribution()
    )

    time_df = (
        scene_stats.timeofday_distribution()
    )

    scene_df = (
        scene_stats.scene_distribution()
    )

    weather_df.to_csv(
        report_dir /
        f"{split_name}_weather_distribution.csv",
        index=False
    )

    time_df.to_csv(
        report_dir /
        f"{split_name}_timeofday_distribution.csv",
        index=False
    )

    scene_df.to_csv(
        report_dir /
        f"{split_name}_scene_distribution.csv",
        index=False
    )
    ...
    plot_category_distribution(
        ...
        figure_dir /
        f"{split_name}_weather_distribution.png"
    )
    ...
    plot_category_distribution(
        ...
        figure_dir /
        f"{split_name}_timeofday_distribution.png"
    )
    ...
    plot_category_distribution(
        ...
        figure_dir /
        f"{split_name}_scene_distribution.png"
    )
